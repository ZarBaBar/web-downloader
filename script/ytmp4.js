export const ytmp4 = async (url, result) => {
  const END_POINT = `https://api.ryzendesu.vip/api/downloader/ytmp4?url=${encodeURIComponent(
    url
  )}`;
  try {
    ...
    result.innerHTML = `<div class="result">
          ...
          <a href="${data.downloadUrl}" class="download"><button>Download file mp4...</button></a>
      </div>`;
  } catch (error) {
    console.error(error);
